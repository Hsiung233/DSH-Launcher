...
{
    /// <summary>
    /// 启动失败的诊断:失败输出 → 可操作提示 → "依赖层快照"留证。
    /// <para>
    /// 为什么单独成类:这一块只在**失败路径**上跑,与进程生命周期、版本管理完全没有耦合,
    /// 却有 400 行左右;放在 <see cref="DshService"/> 里会把"启动/停止"的主线淹没在诊断细节中。
    /// 它本身也不需要服务的状态 —— 需要的东西(shim 路径、profile 名)由调用方传进来,
    /// 因此全是可直接测试的纯函数 + 只读探测(见 DSH Launcher.Tests)。
    /// </para>
    /// </summary>
    internal static class StartFailureDiagnostics
    {
        /// <summary>
        /// 分析启动失败的输出,识别常见错误模式并给出可操作的建议。
        /// </summary>
        /// <param name="output">进程输出。</param>
        /// <param name="packageName">全局包名(提示里让用户重装的那个)。</param>
        internal static string AnalyzeFailureOutput(string output, string packageName)
        {
            var hints = new List<string>();

            // 端口被占用(EADDRINUSE / Windows 套接字提示),尝试提取端口号。
            // ⚠ 取法:先在关键字之后的**同一行**里找"冒号 + 2-5 位数字",取不到再退回"关键字后第一串数字"。
            //    不能只用后者:Node 的典型输出是
            //    "Error: listen EADDRINUSE: address already in use 127.0.0.1:3080",
            //    其中第一串数字是 IP 里的 127 —— 旧写法会提示"端口 127 已被占用",
            //    把用户引到完全错误的排查方向(单元测试锁住了这条)。
            var portMatch = Regex.Match(
                output,
                @"(?:EADDRINUSE|address already in use|套接字地址)[^\r\n]*?[:：](\d{2,5})\b",
        ...
        internal static bool IsProfileResolutionFailure(string? output)
        ...
        /// 正常路径已由启动时的"逃逸上游进程链"处理(见 <see cref="CompatChainEscape"/>,它不依赖
        /// 安装器侧的任何修复),所以这里只描述现象并给出下一步。
        /// </para>
        /// </summary>
        internal static string ProfileResolutionHintFor(string output)
        {
        ...
        /// </summary>
        private const int PackageLimit = 20;
        ...
        /// </para>
        /// </summary>
        /// <param name="failureOutput">失败时的进程输出(用于提取缺失包名与 profile 目录)。</param>
        /// <param name="profileName">dsh 的 profile 名(缺省路径用)。</param>
        /// <param name="shimPath">dsh 命令路径(由调用方定位后传入;空串表示没定位到)。</param>
        internal static void WriteSnapshot(string failureOutput, string profileName, string shimPath)
        {
            try
            {
                ...
                var globalDsh = TryGetGlobalDshDirFromShim(shimPath);
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        lines.Add(broken.Count > 0
                            ? $"  ⚠ 声明依赖 {declared.Count} 项中有 {broken.Count} 项在全局 node_modules 解析不到:{string.Join(", ", broken.Take(PackageLimit))}"
                ...
                else
                {
                    ...
                    lines.Add($"② 共享农场:{farmDir} / 共 {CountEntries(farmDir)} 项,悬空 junction {dangling.Count} 项{(dangling.Count > 0 ? ":" + string.Join(", ", dangling.Take(PackageLimit)) : string.Empty)}");
                ...
                {
                    lines.Add($"  ⚠ 悬空链接 {webDangling.Count} 项:{string.Join(", ", webDangling.Take(PackageLimit))}");
                }
                foreach (var pkg in missing.Take(3))
                {
                ...
                lines.Add($"④ .dsh-module-fallback:{DescribeDirectory(fallbackDir)}");
            ...
            static string DescribeDirectory(string dir) =>
        ...
        private static string? TryGetGlobalDshDirFromShim(string shimPath)
        {
            try
            {
