...
namespace DSH_Launcher.Services
{
    ...
    public sealed partial class DshService
    {
        ...
        /// 日志面板正文的字符数上限(只保留尾部)。
        /// <para>
        /// ⚠ 这个上限不是"省内存"的锦上添花,而是**防界面假死**的硬要求:dsh 启动失败时 Node 会把
        ...
        /// <remarks>
        /// <para>
        /// 真正的根因(2026-09-18 端到端实验闭环):通过安装器“安装完成后启动”拉起的实例处于
        /// **安装器进程链**里,Windows 在加载器层随链继承的 AppCompat shim 本体让 launcher 派生的
        /// <c>cmd</c>/<c>node</c>(dsh) 对 profile 依赖的 junction 大面积不可达 ⇒ 整片
        /// <c>Cannot find package '@deepseek-ai/*'</c>。清子进程环境变量拦不住(载体是进程内的 shim,
        /// 环境变量只是影子),正解是在启动时**逃逸出安装器链**重开自己
        /// (见 App.axaml.cs 的 IsInsideInstallerCompatChain / RelaunchOutsideInstallerChain)。
        /// </para>
        /// <para>
        ...
        /// <para>
        /// 真因(2026-09-18 端到端实验闭环):处于**安装器进程链**里的实例(由 setup 的"安装完成后启动"拉起),
        /// Windows 在加载器层随链继承的 AppCompat shim 让 dsh 对 profile 依赖的 junction 大面积不可达;
        /// 清环境变量拦不住。正常路径已由启动时的"逃逸安装器链"处理(见 App.axaml.cs),
        /// 所以这里只描述现象并给出下一步。
        /// </para>
        /// </summary>
        private static string ProfileResolutionHintFor(string output)
        {
            ...
            return "• dsh 解析不到自己的 profile 依赖(整片 \"Cannot find package\"):"
                + "已确认的成因是**安装器进程链**带来的链级兼容层标记(进程内 AppCompat shim 随链继承),"
                + "它会让 dsh 看不到 profile 依赖目录里的链接。\r\n"
                + "  本程序启动时会自动逃逸出安装器链并重开自己;看到这条提示说明当前实例仍在链里 —— "
                + "请关闭本程序后改用开始菜单/桌面图标重新打开(不要用安装完成时的“启动应用”)。\r\n"
